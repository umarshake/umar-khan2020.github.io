...
function getPageByAjax(page){
	...
	$.ajax({
        ...
        success: function (response){
            $('.content-area').html(response).hide().fadeIn("slow",function(){console.log("animated")});
        },
        error: function (response){
            $('.content-area').html(response.responseText).hide().fadeIn(3000,function(){console.log("animated")});
        } 
    });
}
